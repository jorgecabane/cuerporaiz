...
import {
  ...
  BankLogo,
  ErrorBanner,
} from "../_shared/checkout-ui";
import { continueWithMercadoPago, submitTransferClaim } from "./actions";
...
export function CheckoutOrderClient(props: Props) {
  ...
  function handleContinueMp() {
    ...
    startTransition(async () => {
      const r = await continueWithMercadoPago(props.orderId);
  ...
  function handleSubmitTransfer(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const r = await submitTransferClaim(fd);
      if (r?.error) setError(r.error);
    });
  }
    ...
    <div className="bg-[var(--color-surface)]">
      <div className="mx-auto grid max-w-[1100px] grid-cols-1 lg:grid-cols-[1fr_380px]">
        <main className="order-2 border-b border-[var(--color-border)] px-6 py-8 sm:px-10 sm:py-10 lg:order-none lg:border-b-0 lg:border-r">
          <h1 className="text-[22px] font-semibold tracking-tight text-[var(--color-text)]">Pagar</h1>
          <p className="mt-1 mb-7 text-sm text-[var(--color-text-muted)]">
            Elige cómo quieres pagar tu plan.
          </p>
          ...
          {props.mpEnabled && (
            ...
            >
              <div className="rounded-[var(--radius-md)] bg-[#F7F7F4] p-4 text-sm text-[var(--color-text)]">
                ...
                Tu plan se activa apenas se confirme la transacción.
              </div>
              <button
                type="button"
          ...
          {props.transferAvailable && (
            <MethodCard
              ...
              subtitle="El centro confirma manualmente · puede tardar unas horas"
            >
              <form onSubmit={handleSubmitTransfer} className="space-y-4">
                ...
                <p className="text-center text-xs text-[var(--color-text-muted)]">
                  Te avisaremos por mail cuando el centro lo apruebe.
                </p>
              </form>
            </MethodCard>
          )}
        ...
        <aside className="order-1 bg-[#FAFAF8] px-6 py-8 sm:px-8 sm:py-10 lg:order-none">
          <div className="border-b border-[var(--color-border)] pb-3">
            <div className="flex items-start justify-between gap-3 text-sm">
              <div>
                <div className="font-medium text-[var(--color-text)]">{props.planName}</div>
                <div className="mt-0.5 text-xs text-[var(--color-text-muted)]">
                  {props.centerName} · {props.planMeta}
                </div>
              </div>
          ...
          <p className="mt-6 text-xs leading-relaxed text-[var(--color-text-muted)]">
            Si pagas por transferencia, tu plan queda en estado pendiente hasta que el centro confirme.
            Te avisaremos por mail apenas se apruebe.
          </p>
        </aside>
      </div>
    </div>
