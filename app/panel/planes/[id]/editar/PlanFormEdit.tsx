...
function getInitialValidityKind(plan: Plan): ValidityKind | "" {
  if (plan.validityDays != null) return "days";
  if (plan.validityPeriod != null) return "period";
  return "";
}

function getInitialUsesLimit(plan: Plan): UsesLimit {
  return plan.maxReservations == null ? "unlimited" : "limited";
}
...
export function PlanFormEdit({
  ...
  const [validityDaysInput, setValidityDaysInput] = useState(String(plan.validityDays ?? ""));
  const [validityPeriodInput, setValidityPeriodInput] = useState<ValidityPeriod | "">((plan.validityPeriod as ValidityPeriod) ?? "");
  const [planType, setPlanType] = useState<PlanType>(plan.type as PlanType);
  const [usesLimit, setUsesLimit] = useState<UsesLimit>(getInitialUsesLimit(plan));
          ...
          {usesLimit === "limited" && (
            <div className="mt-2 flex items-center gap-2 flex-wrap">
              <input
                ...
                type="number"
                min={1}
                defaultValue={plan.maxReservations ?? ""}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                type="number"
                min={1}
                defaultValue={plan.maxReservationsPerDay ?? ""}
                className="w-full rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-sm text-[var(--color-text)]"
              />
            </div>
            <div>
              ...
              <input
                ...
                type="number"
                min={1}
                defaultValue={plan.maxReservationsPerWeek ?? ""}
                className="w-full rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-sm text-[var(--color-text)]"
              />
            </div>
          </div>
      ...
      {showRecurringDiscount && (
        <div>
          ...
          <input
            ...
            max={100}
            placeholder="0"
            defaultValue={plan.recurringDiscountPercent ?? ""}
            className="w-full rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-[var(--color-text)]"
          />
          <p className="mt-1 text-xs text-[var(--color-text-muted)]">
